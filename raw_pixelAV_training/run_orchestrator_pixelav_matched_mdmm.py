"""
Orchestrator for the pixelAV-matched Stage 1 MDMM threshold search.
No TF imports -- the subprocess gets the full GPU memory budget.
Adapted from ADC_effect_training/mdmm/2ns5ns/run_orchestrator_2ns5ns_mdmm.py.
...
This is a fresh campaign, independent of train_vit_rnd_thr_pixelav_matched.py's
non-MDMM runs (fp 47f2d3a6, 452882491) -- MDMM changes the loss landscape, so
those runs' thresholds aren't comparable and aren't counted here.
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("run_orchestrator_pixelav_matched_mdmm.out"),
        logging.StreamHandler(),
    ],
)

TRAINED_MODELS_DIR = "/work/projects/SmartPixML/dataset_3srb_16x16_50x12P5_centeredIncidence/trained_models_rnd_thr_mdmm"
JSONL_PATH = os.path.join(TRAINED_MODELS_DIR, "threshold_runs_pixelav_matched_mdmm.jsonl")
MEDIAN_PATH = os.path.join(TRAINED_MODELS_DIR, "median_thresholds_pixelav_matched_mdmm.json")
TRAINING_SCRIPT = os.path.join(
    os.path.dirname(os.path.abspath(__file__)),
    "train_loop_pixelav_matched_mdmm.py",
)
...
def write_median():
    ...
    summary = {
        ...
        "note": "median over non-stuck runs; per-run details in threshold_runs_pixelav_matched_mdmm.jsonl",
    }
...
def main():
    logging.info("=== pixelAV-matched MDMM orchestrator started (no TF, random seeds, journal-driven resume) ===")
