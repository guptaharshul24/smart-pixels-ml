"""
Evaluate the pixelAV-matched Stage 2.5 (QKeras-quantized Conv2D, frozen
hard-digitized thresholds, no-noise TFRs, MDMM) run: residuals, pulls, and
summary plots. Same pattern as eval_pixelav_matched_part2_mdmm.py, but for
the QConv2D architecture -- requires TF_USE_LEGACY_KERAS=1 before any
TF/Keras/QKeras import (QKeras 0.9.0 needs legacy Keras 2, see
models/models.py and models/mdmm.py for the full verification) and
run_eagerly=True at compile time (QSeparableConv2D's quantizer calls
.numpy() internally, which breaks under graph-mode tracing).
"""
import os
# Must be set before any TF/Keras import -- see models/models.py's CreateModel
# (unconditionally tf_keras) and train_qconv2d_part2p5_pixelav_matched_mdmm.py's
# docstring for why. Without this, tf.keras.optimizers.Nadam(...) below would
# resolve to this env's default Keras 3 instead, mixing two Keras runtimes in
# one model.
...
CASE_TAG = "pixelAV-matched, Stage 2.5 QConv2D, frozen thresholds, no_noise, MDMM"
...
if not summary_paths:
    raise SystemExit(f"No Stage 2.5 summary.json found under {part2p5_output_dir} -- has a run completed yet?")
