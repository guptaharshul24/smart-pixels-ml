"""Distributions of the raw predicted cotA/cotB vs truth for a pixelAV-matched
Stage 1 run. Reads the predictions.csv saved by eval_pixelav_matched_rnd_thr.py
(must be run first for the same --fingerprint). Same pattern as every other
plot_pred_angle_dists_*.py in this repo -- the standard collapse diagnostic.

Usage: python plot_pred_angle_dists_pixelav_matched_rnd_thr.py --fingerprint <fp>
"""
import os
import argparse
import pandas as pd
...
if not os.path.exists(pred_path):
    raise SystemExit(f"No predictions.csv for {fingerprint} -- run "
                      f"eval_pixelav_matched_rnd_thr.py --fingerprint {fingerprint} first.")
...
fig.suptitle(f"Predicted vs true angle distributions: pixelAV-matched Stage 1 ViT rnd_thr ({fingerprint})")
plt.tight_layout()
