...
Stage 1 run. Reads the predictions.csv saved by eval_pixelav_matched_mdmm.py
...
if not os.path.exists(pred_path):
    raise SystemExit(f"No predictions.csv for {fingerprint} -- run "
                      f"eval_pixelav_matched_mdmm.py --fingerprint {fingerprint} first.")
...
fig.suptitle(f"Predicted vs true angle distributions: pixelAV-matched Stage 1 ViT rnd_thr MDMM ({fingerprint})")
plt.tight_layout()
