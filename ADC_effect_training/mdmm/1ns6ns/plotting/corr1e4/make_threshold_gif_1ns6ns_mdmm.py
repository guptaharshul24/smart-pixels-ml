...
Same data/styling as plot_thresholds_1ns6ns_mdmm.py (faint per-run lines +
bold cross-run median) but animated instead of a single static frame.

Usage: python make_threshold_gif_1ns6ns_mdmm.py [--frames 200] [--fps 20]
"""
import os
import csv
import glob
...
def update(i):
    ...
    ax.set_title(f"Threshold convergence: {n_runs} run(s), 1ns/6ns, epoch {upto}")
