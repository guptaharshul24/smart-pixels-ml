"""Distributions of the raw predicted cotA/cotB vs truth for a given run.
Reads the predictions.csv saved by eval_transformer_1ns6ns_mdmm.py (must be run
first for the same --fingerprint). Checks whether any dispersion the model
shows is honest, input-dependent spread or a gamed/uncorrelated one.

Usage: python plot_pred_angle_dists_1ns6ns_mdmm.py --fingerprint <fp>
"""
import os
import argparse
import pandas as pd
...
if not os.path.exists(pred_path):
    raise SystemExit(f"No predictions.csv for {fingerprint} -- run "
                      f"eval_transformer_1ns6ns_mdmm.py --fingerprint {fingerprint} first.")
...
fig.suptitle(f"Predicted vs true angle distributions: correlation-constraint 1e4 1ns/6ns "
             f"transformer ({fingerprint})")
plt.tight_layout()
