...
Reads the predictions.csv saved by eval_transformer_2ns5ns_mdmm.py (must be run
...
Usage: python plot_pred_angle_dists_2ns5ns_mdmm.py --fingerprint <fp>
"""
import os
import argparse
import pandas as pd
...
if not os.path.exists(pred_path):
    raise SystemExit(f"No predictions.csv for {fingerprint} -- run "
                      f"eval_transformer_2ns5ns_mdmm.py --fingerprint {fingerprint} first.")
...
fig.suptitle(f"Predicted vs true angle distributions: correlation-constraint 1e4 2ns/5ns "
