...
plot_mdmm_state_2ns5ns_mdmm.py that produced mdmm_state_2ns5ns_mdmm_std1e4.png
...
applies to the archive_scale1/ campaign (same std-constraint log format,
different scale/config at runtime only).
...
trained_models_dir = "/home/harshul-cern/work/projects/SmartPixML/dataset_3srb_16x16_50x12P5_centeredIncidence_10ps_300k_convolved_to_200ps/shuffled_3d/trained_models_2_5_noise_corr_contained_2ns5ns_mdmm_std1e4"
...
fig.suptitle(f"MDMM state (std constraints, scale=1e4 ARCHIVE): corr noise + contained + 2ns/5ns, {len(per_run)} run(s)")
plt.tight_layout()
out = os.path.join(os.path.dirname(os.path.abspath(__file__)), "mdmm_state_2ns5ns_mdmm_std1e4.png")
