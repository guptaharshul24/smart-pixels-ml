...
NLL) is plotted instead when present, so curves stay comparable across
MDMM and non-MDMM runs. Discovers runs via checkpoint dirs directly (not
summary.json, which is only written on completion) so in-progress runs show up.
