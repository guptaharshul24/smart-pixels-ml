"""Distributions of the raw predicted cotA/cotB vs truth for a given Part 1.5 run.
Reads the predictions.csv saved by eval_part1p5_2ns5ns.py (must be run first for
the same --fingerprint). Checks whether any dispersion the model shows is
honest, input-dependent spread or a gamed/uncorrelated one -- this is the
diagnostic that first caught the angle collapse in Part 1.5's pre-MDMM attempt.

Usage: python plot_pred_angle_dists_part1p5_2ns5ns.py --fingerprint <fp>
"""
import os
import argparse
import pandas as pd
...
if not os.path.exists(pred_path):
    raise SystemExit(f"No predictions.csv for {fingerprint} -- run "
                      f"eval_part1p5_2ns5ns.py --fingerprint {fingerprint} first.")
...
fig.suptitle(f"Predicted vs true angle distributions: Part 1.5 ViT 2ns/5ns ({fingerprint})")
plt.tight_layout()
