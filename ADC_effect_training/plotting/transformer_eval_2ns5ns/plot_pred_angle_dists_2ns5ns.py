"""Distributions of the raw predicted cotA/cotB vs truth for a given Part-1
(no-MDMM, threshold-search) 2ns/5ns transformer run. Reads the predictions.csv
saved by eval_transformer_2ns5ns.py (must be run first for the same
--fingerprint).

Usage: python plot_pred_angle_dists_2ns5ns.py --fingerprint <fp>
"""
import os
import argparse
import pandas as pd
...
if not os.path.exists(pred_path):
    raise SystemExit(f"No predictions.csv for {fingerprint} -- run "
                      f"eval_transformer_2ns5ns.py --fingerprint {fingerprint} first.")
...
fig.suptitle(f"Predicted vs true angle distributions: Part 1 (no MDMM) transformer 2ns/5ns ({fingerprint})")
plt.tight_layout()
